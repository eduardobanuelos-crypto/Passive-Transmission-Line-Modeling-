function [A_matrix, B_matrix, C_matrix, D_matrix] = State_space_matrices_for_passivity(Data)
%{
...
%}
	...
	B = [-R_in/L_in,    zeros(1,1),   zeros(1,1);
		  zeros(M,1),   zeros(M,1),   zeros(M,1);
		  zeros(M*m,1), zeros(M*m,1), zeros(M*m,1);
			  0,         1/C_in,           0;
		  zeros(M-1,1), zeros(M-1,1), zeros(M-1,1)
			  0,             0,       1/C_fin];
	B_matrix.B = B; % Structure

	% Matrix C
	cap_in = n_tot - M - 1;
	C_mat = [-R_in,   zeros(1,   A_size-1);
			 zeros(1, cap_in),   1, zeros(1,M);
			 zeros(1, A_size-1), 1];
